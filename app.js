...
app.post('/save',(req,res) => {
    let data = {name: req.body.name, class: req.body.email, regno: req.body.phone};
...
app.get('/edit/:id',(req, res)=>{
    ...
    let sql = `SELECT * FROM sources where id = ${userId}`;
...
app.post('/update',(req, res)=>{
    ...
    let sql = "update sources SET name='"+req.body.name+"', class='"+req.body.class+"', regno='"+req.body.regno+"' where id = "+userId;
...
app.get('/delete/:id',(req, res)=>{
    const userId = req.params.id;
    let sql = `delete FROM sources where id = ${userId}`;
    let query = connection.query(sql,(err, result)=>{
        if(err) throw err;
        res.redirect('/');
    });
...
app.post('/ss',(req, res) => {
   ...
   res.send("<h1>Invalid Input</h1>");
    ...
    file.mv("./uploads/"+filename);
        ...
        let sql = `SELECT source_name FROM sources`;
        let query = connection.query(sql, (err, result) => {
            ...
            Object.keys(result).forEach(function (key) {
                ...
                for (let keyss in data) {
                    if (data[keyss]['source_name'] !== row.source_name) {
